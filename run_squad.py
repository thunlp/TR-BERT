...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter("logs/squad2_log/" + args.output_dir[args.output_dir.find('/')+1:])
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                "input_ids": batch[0],
                "attention_mask": batch[1],
                "token_type_ids": None if args.model_type in ["xlm", "roberta", "distilbert", "distilautobert"] else batch[2],
                "start_positions": batch[3],
                "end_positions": batch[4],
                "copy_rate": copy_rate, 
                "tokens_prob": batch[-1],
...
def evaluate_logits(args, model, tokenizer, prefix="", evaluate=False):
    ...
    np.save("npy_folder/squad_logits.npy", all_logits)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if not evaluate:
        ...
        if args.train_rl:
            ...
            tokens_prob = np.load(os.path.join("npy_folder", "squad_" + model_type + "_"+ str(args.max_seq_length)+ "_score.npy"))
            ...
            tokens_rank = np.load(os.path.join("npy_folder", "squad_" + model_type + "_"+ str(args.max_seq_length)+ "_rank.npy"))
...
def evaluate_grad(args, model, tokenizer, prefix="", evaluate=False):
    ...
    np.save(os.path.join("npy_folder", "squad_" + args.model_type + "_"+ str(max_seq_length)+ "_score.npy"), layers_scores)
...
def main():
    ...
    parser.add_argument("--teacher_npy_path",
                        default="npy_folder/squad_logits.npy",
                        type=str,
                        required=False)
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test')==-1:
        create_exp_dir(args.output_dir, scripts_to_save=['run_squad.py',  
        'transformers/src/transformers/modeling_distilautoqabert.py', 'transformers/src/transformers/modeling_autoqabert.py',])
