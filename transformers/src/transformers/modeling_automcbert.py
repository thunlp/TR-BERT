...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        w = 0
        mini = 1#16     # if the sequence length is too small, the acceleration is meanfulless
        mid = len(self.layer)//2
        argbsz = torch.arange(bsz).unsqueeze(-1)
        argoribsz = torch.arange(ori_bsz).unsqueeze(-1)
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if (i in [1, mid]) and (num_items > mini):
                ...
                if copy_rate is None:
                    ...
                    sample_prob += max_seq_mask - question_ends_mask
                    sample_prob[:, 0] = 2
                    selected_token = (sample_prob >= 0.5).to(max_seq_mask) * max_seq_mask
                else:
                    if copy_rate.sum()>0:
                        with torch.no_grad():
                            if tot_zoom is None:
                ...
                top_indices_expand = top_indices.view(1, bsz, num_items_to_keep).repeat(option_nums, 1, 1)
                top_indices_expand = top_indices_expand.view(ori_bsz, num_items_to_keep)
                question_ends_mask = question_ends_mask[argbsz, top_indices]
                seq_mask = seq_mask[argoribsz, top_indices_expand]
                hidden_states = hidden_states[argoribsz, top_indices_expand]
...
class AUTOMCBertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
