...
class BertForSequenceClassificationSigmoid(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = loss_fct(logits, labels.to(logits))#-torch.nn.functional.logsigmoid(logits).mean()
            outputs = (loss,) + outputs
...
class BertForMultipleChoiceSigmoid(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            new_labels = torch.zeros(bsz, 4).to(input_ids)
            new_labels[torch.arange(bsz).to(input_ids), labels] = 1
            loss_fct = BCEWithLogitsLoss(reduction='mean')
            loss = loss_fct(reshaped_logits, new_labels.to(reshaped_logits))
...
class BertForMultipleChoiceSoftmax(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        reshaped_logits = reshaped_logits.view(-1, num_choices)
        ...
        if labels is not None:
            ...
            loss = loss_fct(logits, new_labels.view(-1))
